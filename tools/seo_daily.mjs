...
書き方の約束:
- **導入事例・お客様の声・調査結果を作ってはいけない。** 実在しないサロンの成果
  （「導入後3ヶ月で来客数が◯人増えた」）、出典のない統計（「◯%が〜と回答」
  「業界平均は◯件」）、書かれていないオーナーのコメントは、たとえ「よくある例」の
  つもりでも書かない。過去にこれで嘘が載った。
  稼働している事例は BONHEUR HAIR（東京都足立区）と ZiL 亀有（東京都葛飾区）の2店のみで、
  **公開してよい成果の数字は無い**。事例に触れるなら、何をしているか（施術中の一次応対、
  ホットペッパー併用時の予約一元管理）という事実だけを書く。
- 効果は「読者が自分の数字で見積もる手順」として書く。断定した結果として書かない。
  例:「月◯件増えます」ではなく「着信履歴から出られなかった件数を数えて、新規率と
  客単価を掛けてください」。
- 相場や一般論を出すときは「〜と言われる」など、根拠の強さに応じた書き分けをする。
...
- 料金は「プラン」ではなく **使う機能ごとの月額の合計**（電話 / 集客 / LINE会員 / サーベイ の4種）。
  月額・税抜で 集客 ¥2,980 / LINE会員 ¥2,980 / サーベイ ¥2,980（1店舗あたり・店舗が増えるほど割引）。
  電話（MOYO Call）は月の通話件数で決まる: お試し 10件まで ¥3,500 / スタンダード 50件まで ¥5,000 /
  ビジネス 200件まで ¥14,800 / 超過 ¥100 per 件。
  4つすべてなら全部入りセット: 電話50件で ¥11,800 / 電話200件で ¥19,800（お試し10件は対象外）。
  例: 電話のみ月10件まで ¥3,500、電話のみ月50件まで ¥5,000、電話（月200件）＋LINE会員 ¥17,780、4つすべて・月50件 ¥11,800。
  AI専用番号の維持費 ¥739/月（税抜・為替連動）が別途。
  **「機能の数で決まる ¥5,000/¥9,000/¥13,000/¥15,000」「Starter / Growth」は旧体系。もう使わない**
